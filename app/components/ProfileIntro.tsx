...
export default function ProfileIntro({ showButtons = false }) {
  return (
    <div className="relative pt-16 pb-10 md:py-32 flex flex-col md:flex-row items-center gap-8 md:gap-20 px-4 md:px-0">
      ...
      <motion.div
        ...
        className="flex-1 space-y-6 md:space-y-8 text-center md:text-left z-10 w-full"
      >
        <div className="inline-flex flex-wrap justify-center gap-3 md:justify-start">
          {badges.map((badge, idx) => (
            <motion.span
              ...
              key={badge}
              className="rounded-full bg-slate-100 dark:bg-slate-800 px-3 py-1 text-[10px] md:text-xs font-semibold uppercase tracking-wider text-slate-600 dark:text-slate-300 shadow-sm border border-slate-200 dark:border-slate-700"
            >
              {badge}
            </motion.span>
          ))}
        ...
        <div className="space-y-2 md:space-y-4">
          <h2 className="text-base md:text-xl font-medium text-slate-500 dark:text-slate-400">
            Hello, I am
          </h2>
          <h1 className="text-4xl sm:text-5xl md:text-7xl font-display font-extrabold text-slate-900 dark:text-white leading-[1.1]">
        ...
        <p className="text-base md:text-xl leading-relaxed text-slate-600 dark:text-slate-300 max-w-2xl mx-auto md:mx-0">
      ...
      <motion.div
        ...
        className="relative flex-shrink-0 z-10 w-full max-w-[280px] md:max-w-md"
      >
        <div className="relative w-full aspect-square">
          <div className="absolute inset-0 bg-gradient-to-tr from-primary to-accent rounded-[2rem] md:rounded-[3rem] rotate-6 opacity-30 blur-lg animate-pulse" />
          <div className="absolute inset-0 bg-gradient-to-tr from-primary to-accent rounded-[2rem] md:rounded-[3rem] -rotate-3 transition-transform duration-500 hover:rotate-0" />
          
          <div className="absolute inset-1 rounded-[2rem] md:rounded-[3rem] overflow-hidden bg-slate-900 border-4 border-white dark:border-slate-800">
            <Image
            ...
            <div className="absolute bottom-4 left-4 right-4 glass px-4 py-2 rounded-2xl flex items-center justify-between">
              <span className="text-[10px] md:text-xs font-semibold uppercase tracking-wider text-slate-800 dark:text-white">
                Kigali, Rwanda
              </span>
